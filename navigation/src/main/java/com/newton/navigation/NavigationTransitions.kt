...
class NavigationTransitions {
    ...
    fun getEnterTransition(
        ...
        return {
            when (type) {
                TransitionType.SLIDE_HORIZONTAL -> slideIntoContainer(
                    towards = AnimatedContentTransitionScope.SlideDirection.Left,
                    animationSpec = tween(durationMillis = durationMs, easing = EaseIn)
                )

                TransitionType.SLIDE_VERTICAL -> slideIntoContainer(
    ...
    fun getExitTransition(
        ...
        return {
            when (type) {
                TransitionType.SLIDE_HORIZONTAL -> slideOutOfContainer(
                    towards = AnimatedContentTransitionScope.SlideDirection.Left,
                    animationSpec = tween(durationMillis = durationMs, easing = EaseOut)
                )

                TransitionType.SLIDE_VERTICAL -> slideOutOfContainer(
                    towards = AnimatedContentTransitionScope.SlideDirection.Up,
                    animationSpec = tween(durationMillis = durationMs, easing = EaseOut)
                )
